...
| `nuxtUiHelper.openFromVSCode` | [registerCommands.ts](src/activation/registerCommands.ts) | Internal — wired to the inlay hint click; passes full `ComponentContext` |

`showComponentMenu` and `openFromVSCode` receive a `ComponentContext` (`documentUri` + `tagOffset` + `tagName`) so the insertion functions know exactly where to write in the source file.
...
Each function takes a `ComponentContext` + the value to insert and applies a `WorkspaceEdit`.
...
```
ComponentContext (uri + offset + tagName)
